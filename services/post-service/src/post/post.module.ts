...
import { PARTICIPATION_AUTHORIZATION, POST_READ_QUERIES, POST_STATE_QUERIES, POST_UNIT_OF_WORK } from './application/ports.js';
import type { ParticipationAuthorization, PostReadQueries, PostStateQueries, PostUnitOfWork } from './application/ports.js';
...
@Module({
  ...
  providers: [
    ...
    { provide: ReadPosts, useFactory: (queries: PostReadQueries) => new ReadPosts(queries), inject: [POST_READ_QUERIES] },
    ...
    { provide: CreateComment, useFactory: (unitOfWork: PostUnitOfWork, queries: PostStateQueries) => new CreateComment(unitOfWork, queries), inject: [POST_UNIT_OF_WORK, POST_STATE_QUERIES] },
