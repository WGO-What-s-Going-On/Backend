...
export function parseClientCommand(raw: string): ClientCommand {
  ...
  if (
    ...
    (typeof payload.content !== 'string' ||
      ...
      !/^[A-Za-z0-9_-]{1,128}$/.test(payload.mutationId))
  )
    throw new ProtocolError(
      'WS_INVALID_PAYLOAD',
      'Valid content and mutationId are required.',
      requestId,
    );
  if (
    message.type === 'comment.list' &&
    ((payload.cursor !== undefined &&
      (typeof payload.cursor !== 'string' || payload.cursor.length > 1024)) ||
    ...
    payload: {
      ...
      ...(message.type === 'comment.create'
        ? {
            ...
            mutationId: payload.mutationId as string,
          }
        : {}),
      ...(message.type === 'comment.list' && payload.cursor !== undefined
        ? { cursor: payload.cursor as string }
        : {}),
      ...(message.type === 'comment.list' && payload.limit !== undefined
