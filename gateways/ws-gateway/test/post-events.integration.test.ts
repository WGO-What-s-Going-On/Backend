...
suite('distributed post events', () => {
  it('reclaims a pending event after a consumer stops before publishing', async () => {
    ...
    await redis.xGroupCreate('post:events', 'post-realtime', '0', {
      MKSTREAM: true,
    });
    const eventId = randomUUID();
    await redis.xAdd('post:events', '*', {
      eventId,
      eventType: 'PostCreated',
      data: JSON.stringify({
        eventId,
        eventType: 'PostCreated',
        aggregateId: 'post-recovery',
      }),
    });
    ...
    expect((await redis.xPending('post:events', 'post-realtime')).pending).toBe(
      1,
    );
    ...
    try {
      ...
      const recovered = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () => reject(new Error('pending event not reclaimed')),
            8000,
          ),
        ),
      ]);
      ...
      for (
        let i = 0;
        i < 20 &&
        (await redis.xPending('post:events', 'post-realtime')).pending !== 0;
        i++
      )
        await new Promise((resolve) => setTimeout(resolve, 25));
      expect(
        (await redis.xPending('post:events', 'post-realtime')).pending,
      ).toBe(0);
    } finally {
      socket.terminate();
      await app.close();
  ...
  it('delivers all four event types to joined rooms on two gateway instances', async () => {
    ...
    try {
      ...
      for (const [eventType, expected] of [
        ...
        ['PostParticipantJoined', 'post.participant.joined'],
      ] as const) {
        const eventId = randomUUID();
        const payload = {
          eventId,
